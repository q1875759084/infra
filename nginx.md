...
| 服务 | 宿主机端口 | 绑定方式 | 说明 |
|------|-----------|---------|------|
| cowatch-backend（生产） | `8000` | `0.0.0.0:8000:3002` | 安全组未开放，外网不可直连；daibao 容器经 `host-gateway` 访问 |
| cowatch-backend（测试） | `8001` | `0.0.0.0:8001:3002` | 同上 |
| monitor-backend（生产） | `3100` | — | monitor 独立部署，daibao 容器经 `host-gateway` 访问 |
| monitor-backend（测试） | `3101` | — | 同上 |

> **安全语义：** 端口绑定在 `0.0.0.0`，但宿主机安全组未开放 8000/8001，外网无法直连。容器通过 `host-gateway`（宿主机 docker0 网桥 IP）访问这些端口可正常通信。注意：`127.0.0.1` 绑定会导致容器经 `host-gateway` 访问时被拒绝（仅宿主机本地进程可达），因此改为 `0.0.0.0` 绑定。
